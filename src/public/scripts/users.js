...
function addUser() {
  ...
  // Call api
  Http.post('/users/add', data).then(() => displayUsers());
}
...
function submitEdit(ele) {
  ...
  Http.put('/users/update', data).then(() => displayUsers());
}
...
function deleteUser(ele) {
  var id = ele.getAttribute('data-user-id');
  Http.delete('/users/delete/' + id).then(() => displayUsers());
}

// **** Logout **** //

function logoutUser() {
  Http.get('/auth/logout').then(() => (window.location.href = '/'));
}
